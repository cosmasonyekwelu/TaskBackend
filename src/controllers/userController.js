...
exports.getProfile = async (req, res, next) => {
  try {
    const user = await User.findOne({ _id: req.user.id, isActive: true }).select("-password -deletedAt").lean();
...
exports.updateProfile = async (req, res, next) => {
  try {
    ...
    const user = await User.findOneAndUpdate({ _id: req.user.id, isActive: true }, updates, {
      new: true,
      runValidators: true,
